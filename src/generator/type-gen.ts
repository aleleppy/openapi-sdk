...
export class TypeGenerator {
  ...
  build(): string {
    ...
    if (allContent.includes('IntersectionType'))
      swaggerImports.push('IntersectionType');
  ...
  private generateClassBlock(
    ...
    for (const fieldName of fieldNames) {
      ...
      if (this.emittedHelpers.has(className)) continue;

      lines.push(
        ...this.generateFieldClass(
          className,
          fieldName,
          fieldSchema,
          isReq,
  ...
  private generateFieldClass(
    ...
    if (schema.type === 'array' && schema.items) {
      ...
      if (
        ...
        const itemLines = this.generateFieldClass(
          itemClassName,
          fieldName,
          itemSchema,
          true,
    ...
    const example =
      schema.example !== undefined
        ? schema.example
        : this.defaultExample(schema);
    const isDateByExample =
      mode === 'response' &&
      typeof example === 'string' &&
      this.looksLikeDate(example);
    const tsType = schema.enum
      ? toPascalCase(fieldName) + 'Enum'
      : isDateByExample
        ? 'Date'
        : this.mapType(schema, fieldName);
    ...
    for (const param of params) {
      ...
      if (!this.emittedHelpers.has(className)) {
        ...
        const example =
          schema?.example !== undefined
            ? schema.example
            : this.defaultExample(schema ?? { type: 'string' });
        const exampleStr =
          typeof example === 'string'
            ? `'${example}'`
    ...
    if (schema.properties && Object.keys(schema.properties).length > 0) {
      ...
      const fields = Object.entries(schema.properties).map(([key, val]) => {
        const resolved = resolveSchema(
          val as SchemaObject | ReferenceObject,
          this.spec,
        );
